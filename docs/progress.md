...
最後檢視：2026-09-20（依目前原始碼與本輪實際執行的 contracts build、Backend build／unit／coverage／隔離 PostgreSQL／Redis E2E、Frontend type-check／unit／production build，以及 read-only lint 結果核對）。
...
| Project domain | 建置中，read／create／member notification 已形成前端 vertical slice | Project、ProjectMember schema／migration、shared contracts、Repository 與 runtime DTO validation 已建立；`POST /project` 建立 Project 與 OWNER membership，list／members／memberCandidates 提供 read models，`POST /project/addMember` 以 `workspaceMemberId` 由 Project OWNER 直接加入同 Workspace 成員，並在 commit 後推送 `PROJECT_MEMBER_ADDED` 通知；`GET /project/notificationDetail/:notificationId` 與前端詳細 Dialog 已串接。Service／Controller tests 與第一版隔離 E2E 已補；Create 仍回傳 `null`，負向授權、rollback 與真實併行 E2E 仍待補 |
...
- Frontend unit tests 目前覆蓋 signup／login pure form validation、User／Notification／Project Store、Project／WorkspaceInvitation services、通知副作用 handler、邀請回覆卡、Project add-member／notification detail Dialog，以及共用 Alert／Loading；尚未覆蓋真實 route、Socket lifecycle 或瀏覽器 Cookie 行為。
...
- 2026-09-15 執行 `pnpm test:backend:e2e`：3 suites、5 tests 全部通過。當時 runner 套用 7 個 migrations，覆蓋 Auth lifecycle、Workspace Invitation 接受／拒絕，以及 Notification 單筆／全部已讀，完成後移除 containers、network 與 volumes；這筆是歷史紀錄，最新 10 migrations／4 suites 結果見下方 2026-09-20 紀錄。
...
- 上述較早的 2026-09-04／09-08／09-11／09-12／09-15／09-16／09-19 結果是歷史紀錄；目前 HEAD 的自動驗證基準以本輪 2026-09-20 的 build、unit、coverage 與隔離 E2E 紀錄為準。
- 進度只在實際跑過對應指令後標記完成，不以「已有 spec 檔」代替通過結果。

## 下一步

1. 建立 Board／Column／Card schema、migration 與 snapshot read model；同時補 Project 未登入／非 OWNER／跨 Workspace／他人通知存取 E2E、transaction rollback、重複加入真實併行測試與既有資料下 ProjectMember `id` migration。
