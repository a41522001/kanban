...
| `Input` | `frontend/src/components/shared/Input/Input.vue` + `shared/FormField/FormField.vue` |
| `Avatar` | `frontend/src/components/shared/Avatar/Avatar.vue` |
| `Workspace Invite Dialog` | `components/workspace/WorkspaceInviteDialog`；底層仍使用 Dialog、Input、Button |
| `Notification Trigger`／`Notification Dropdown` | `components/notifications/NotificationMenu`；底層使用 DropdownMenu、Badge、ScrollArea、Skeleton |
...
- 設計稿中的元件註記應標出對應實作來源，例如 `shadcn-vue/Dialog` 或 `shared/FormField`。
...
- 頁面負責組合與資料流程；跨功能共用元件放在 `components/shared`，領域互動放在對應 domain 資料夾，shadcn-vue primitive 保留在 `components/ui`。
