...
最後核對：2026-09-21（邀請與通知行為未變；更新目前 13 migrations 的隔離 E2E baseline 與 ProjectView 導頁 context）。此文件區分已實作行為與後續目標。
